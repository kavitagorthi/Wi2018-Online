...
with open(starter_file, 'r')as infile:
    ...
    i = 2 # Skipping the first two words generated by kata_seed.
    kata_length = 25
    tbl_kata_words = []
    while i < kata_length:
        ...
        for i in range(i, kata_length):
            if ((tbl_kata_words[i - 2]).lower() + " " + tbl_kata_words[i - 1].lower()) in kata_dict:
                ...
                kata_next_word = kata_next_word.replace(" ,", ", ")
                tbl_kata_words.append(kata_next_word)
            else:
                # If there are no words to add to the current sentence, end it with a period.
                # Then we break out of this for loop and start the initial sentence generation again.
                tbl_kata_words[i - 1] = tbl_kata_words[i - 1] + ". "
                i = i + 2 # Again, skipping the next two words which will be generated by kata_seed again.
                break
        print(i)
